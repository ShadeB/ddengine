 ...
 */
public class AdamMilazzo implements FieldOfViewCalculator {

    private BiFunction<Integer, Integer, Boolean> _blocksLight;
    private BiConsumer<Integer, Integer>          _setVisible;
    private BiFunction<Integer, Integer, Integer> _getDistance;
    ...
    /// </param>
    public AdamMilazzo(BiFunction<Integer, Integer, Boolean> blocksLight,
                       BiConsumer<Integer, Integer> setVisible,
                       BiFunction<Integer, Integer, Integer> getDistance)
    {
    ...
    public void compute(int x, int y, int rangeLimit) {
        setVisible(x, y);
        for(int octant=0; octant<8; octant++) {
            compute(octant, new Coordinate(x, y), rangeLimit, 1, new Slope(1, 1), new Slope(0, 1));
        }
    }
    ...
    class Slope {
        ...
        public boolean Greater(int y, int x) { return this.Y*x > this.X*y; } // this > y/x
        public boolean GreaterOrEqual(int y, int x) { return this.Y*x >= this.X*y; } // this >= y/x
        public boolean Less(int y, int x) { return this.Y*x < this.X*y; } // this < y/x
    }

    public void compute(int octant, Coordinate origin, int rangeLimit, int x, Slope top, Slope bottom) {
    ...
    boolean blocksLight(int x, int y, int octant, Coordinate origin)
    {
    ...
    void setVisible(int x, int y, int octant, Coordinate origin)
    {
        int nx = origin.x, ny = origin.y;
        switch(octant)
        {
    ...
    private boolean blocksLight(int x, int y) {
        return this._blocksLight.apply(x, y);
    }

    private void setVisible(int x, int y) {
        this._setVisible.accept(x, y);
    }

    private int getDistance(int x, int y) {
        return this._getDistance.apply(x, y);
    }
